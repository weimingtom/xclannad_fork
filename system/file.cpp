...
ARCFILE::ARCFILE(char* aname) {
	...
	if ( (sb.st_mode&S_IFMT) == S_IFDIR) {
		int l = strlen(aname);
		arcname = new char[l+2]; memset(arcname, 0, l+2); strcpy(arcname, aname);
		...
		memset(arcname, 0, strlen(aname)+1);
...
ARCINFO* DIRFILE::MakeARCINFO(ARCFILE_ATOM& atom) {
	...
	memset(new_path, 0, strlen(arcname)+strlen(name)+1);
...
int DIRFILE::CheckFileDeal(void) {
	...
	TCHAR arcnameW[256];
	_tcscpy(arcnameW, arcname);
	_tcscat(arcnameW, TEXT("/*"));
...
void SCN2kFILE::ListupFiles(int fname_len) {
	...
	int i; for (i=0; i<10000; i++) {
		...
		else if (file_version != 0x2712) continue; 
...
int FILESEARCH::InitRoot(char* root) {
	...
	if (root[0] == '~' && root[1] == '/') {
		...
		if (home != 0) {
			...
			memset(new_root, 0, strlen(home)+strlen(root));
...
void FILESEARCH::SetFileInformation(FILETYPE tp, ARCTYPE is_arc, char* filename) {
	...
	memset(filenames[type], 0, strlen(filename)+1);
...
char** FILESEARCH::ListAll(FILETYPE type) {
	...
	for (file = searcher[type]; file != 0; file = file->Next()) {
		...
		while( (f = file->ListItem() ) != 0) {
			...
			memset(ret_list[count], 0, strlen(f)+1);
...
ARCINFO::ARCINFO(const char* __arcname, ARCFILE_ATOM& atom) : info(atom) {
	...
	memset(arcfile, 0, strlen(__arcname)+1);
...
char* ARCINFO::CopyRead(void) {
	...
	int s = Size();
	if (s <= 0) return 0;
	char* ret = new char[s]; memset(ret, 0, s); memcpy(ret, d, s);
	return ret;
}
...
template<class DataType, class DataSize> inline int lzExtract(DataType& datatype,const char*& src, char*& dest, const char* srcend, char* destend) {
	...
	if (lsrc+50 < lsrcend && ldest+1024 < ldestend) {
		...
		while (ldest < ldestend && lsrc < lsrcend) {
			...
			if (datatype.IsRev()) {
#if 0
				dprintf("===============>%d, %d, %d\n", flag, bitrev_table[flag], lsrc - src);
#endif

				flag = bitrev_table[flag];
			}
...
bool PDTCONV::Read(char* image) {
	...
	memset(buf, 0, width*height+1024);
...
bool ARCINFO_AVG32::ExecExtract(void) {
	...
	memset(ret_data, 0, info.filesize+1024);
...
bool ARCINFO2k::ExecExtract(void) {
	...
	memset(decoded_data, 0, info.arcsize + 1024);
...
bool G00CONV::Read_Type0(char* image) {
	...
	memset(uncompress_data, 0, uncompress_size+1024);
...
#if 0
	if (uncompress_size == 2564) {
		fprintf(stderr, "==================uncompress_size\n");
	}
#endif
	char* uncompress_data = new char[uncompress_size + 1024];
	memset(uncompress_data, 0, uncompress_size + 1024);
...
#if 0
	fp = fopen("kout", "wb");
	fwrite(uncompress_data, 1, uncompress_size + 1024 * 1, fp);
	fclose(fp);
#endif
...
bool BMPCONV::Read(char* image) {
	...
	memset(buf, 0, width*height*bpp+1024);
