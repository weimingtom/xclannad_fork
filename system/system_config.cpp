...
HashStr::HashStr(const HashStr& orig) {
	...
	memset(new_str, 0, strlen(orig.str)+1);
...
public:
	...
	void DiffOriginal(string& data) {
		...
		memset(out_data, 0, strlen(new_data)*2+1);
		char* buf = out_data;
		int i;
...
public:
	...
	const int* GetOriginal(int deal) const {
		if (item_deal == 0) return 0;
		if (deal > item_deal) {
			fprintf(stderr,"AyuSysConfigIntlistItem::Get : invalid items deal %d (correct: %d)\n",deal,item_deal);
			return 0;
...
int AyuSysConfig::SearchParam(const char* name) const{
	...
	if (str_config->orig.Get(1, str2)) return 1; 
	else if (int_config->orig.Get(1, str2)) return 2; 
	else return 0;
}
...
AyuSysConfig::AyuSysConfig(void) {
	...
	SetOrigParaStr("#REGNAME", "xclannad");	
	SetOrigParaStr("#CAPTION", "xclannad");	
	...
	SetOrigParaStr("#CGTABLE_FILE", "MODE.CGM");

	SetOrigParaStr("#WAKU.000.000.NAME", "");
	SetOrigParaStr("#WAKU.000.000.BACK", "");
	SetOrigParaStr("#WAKU.000.000.BTN", "");

	SetOrigParaStr("#MOUSE_CURSOR.000.NAME", "");
	SetOrigParaStr("#CURSOR.000.NAME", "");
	SetOrigParaStr("#SELBTN.000.NAME", "");
	SetOrigParaStr("#SELBTN.000.BACK", "");
	...
	SetOrigParam("#WINDOW_ATTR", 5, -1, 128,128, 190, 0); 
	...
	SetOrigParam("#INIT_MESSAGE_SPEED", 1, 30); 
	SetOrigParam("#INIT_MESSAGE_SPEED_MOD", 1, 0); 
	SetOrigParam("#MESSAGE_KEY_WAIT_USE", 1, 0); 
	SetOrigParam("#MESSAGE_KEY_WAIT_TIME", 1, 1500); 
	...
	SetOrigParam("#WAKU.000.000.MOVE_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.CLEAR_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.READJUMP_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.AUTOMODE_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.MSGBK_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.MSGBKLEFT_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.MSGBKRIGHT_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.EXBTN_000_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.EXBTN_001_BOX", 5, 0, 0, 0, 0, 0); 
	SetOrigParam("#WAKU.000.000.EXBTN_002_BOX", 5, 0, 0, 0, 0, 0); 

	SetOrigParam("#WINDOW.000.MOJI_SIZE", 1, 21); 
	SetOrigParam("#WINDOW.000.MOJI_REP", 2, -1, 2); 
	SetOrigParam("#WINDOW.000.MOJI_CNT", 2, 20, 3); 
	SetOrigParam("#WINDOW.000.MOJI_POS", 4, 100, 0, 180, 40); 
	SetOrigParam("#WINDOW.000.MOJI_SHADOW", 1, 0); 
	SetOrigParam("#WINDOW.000.LUBY_SIZE", 1, 8); 
	SetOrigParam("#WINDOW.000.MOJI_MIN", 2, 8, 1); 
	SetOrigParam("#WINDOW.000.SELCOM_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.POS", 4, 100, 0, 0, 260); 
	SetOrigParam("#WINDOW.000.ATTR_MOD", 1, 0); 
	SetOrigParam("#WINDOW.000.ATTR", 5, -1, -1, -1, -1, -1); 
	
	SetOrigParam("#WINDOW.000.OPEN_ANM_MOD", 1, 0); 
	SetOrigParam("#WINDOW.000.OPEN_ANM_TIME", 1, 500);
	SetOrigParam("#WINDOW.000.CLOSE_ANM_MOD", 1, 0); 
	SetOrigParam("#WINDOW.000.CLOSE_ANM_TIME", 1, 500);
	SetOrigParam("#WINDOW.000.WAKU_SETNO", 1, 0); 
	SetOrigParam("#WINDOW.000.MOVE_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.CLEAR_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.READJUMP_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.AUTOMODE_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.MSGBK_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.MSGBKLEFT_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.MSGBKRIGHT_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.EXBTN_000_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.EXBTN_001_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.EXBTN_002_USE", 1, 0); 
	SetOrigParam("#WINDOW.000.NAME_MOD", 1, 0); 
	SetOrigParam("#WINDOW.000.NAME_MOJI_SIZE", 1, 20); 
	SetOrigParam("#WINDOW.000.NAME_MOJI_POS", 2, 0, 0); 
	SetOrigParam("#WINDOW.000.NAME_MOJI_MIN", 1, 0); 
	SetOrigParam("#WINDOW.000.NAME_CENTERING", 1, 1); 
	SetOrigParam("#WINDOW.000.NAME_POS", 2, 159, 78); 
	SetOrigParam("#WINDOW.000.NAME_WAKU_SETNO", 1, -1); 
	SetOrigParam("#WINDOW.000.FACE.000", 5, 0, 0, 1, 1, 1); 
	SetOrigParam("#WINDOW.000.KEYCUR_MOD", 3, 0, 0, 0); 


	SetOrigParam("#CURSOR.000.SIZE", 2, 0, 0); 
	SetOrigParam("#CURSOR.000.CONT", 1, 50); 
	SetOrigParam("#CURSOR.000.SPEED", 1, 1000); 

	SetOrigParam("#SELBTN.000.BASEPOS", 2, 0, 0); 
	SetOrigParam("#SELBTN.000.REPPOS", 2, 0, 50); 
	SetOrigParam("#SELBTN.000.MOJISIZE", 4, 26, 0,0,0); 
	SetOrigParam("#SELBTN.000.MOJIDEFAULTCOL", 1, 0); 
	SetOrigParam("#SELBTN.000.MOJISELECTCOL", 1, 0); 
...
void TrackName::AddCDROM(char* name, int tk) {
	...
	int num = i;
	if (i == deal) Expand();
	track[num] = new char[strlen(name)+1]; memset(track[num], 0, strlen(name)+1);
...
void TrackName::AddWave(char* name, char* file, int pt) {
	...
	for (i=0; name[i] != 0; i++) track[num][i] = tolower(name[i]);
	track[num][i] = 0;
	track_wave[num] = new char[strlen(file)+1]; memset(track_wave[num], 0, strlen(file)+1); strcpy(track_wave[num], file);
}
...
void TrackName::AddSE(int n, char* file) {
	...
	memset(se_track[n], 0, strlen(file)+1);
