...
private:
	...
	std::vector<int> bottom_pos; 
	std::vector<int> bottom_pos_backup;
	void _save_bottom_pos() { bottom_pos_backup = bottom_pos; }
	int safe_bottom_pos(int index) {
		if (index >= bottom_pos.size()) {
			dprintf("==============>bottom_pos.size() <= %d", index);
			if (index >= bottom_pos_backup.size()) {
				return 0;
			}
			else {
				return bottom_pos_backup[index];
			}
		}
		else {
			return bottom_pos[index];
		}
	}
